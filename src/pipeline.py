...
def process_dataset(
    ...
    if require_more_than_coh and frag_or_cpd == "cpd":
        print(
            "Removing compounds with more than C, O, H is not supported because it is redundant. Try removing only these fragments."
        )
    ...
    if frag_or_cpd == "cpd" and len(remove_patterns) > 0:
        print(
            "Removing patterns from compounds is not supported because it is redundant. Try removing patterns from fragments only."
        )
...
def compile_frag_only_results_into_df(df, mols, result_path, frag_hit_column):
    ...
    rank_df["fragment_scores"] = [
        df.iloc[i, list(df.columns).index(frag_hit_column)] for i in range(len(mols))
    ]
...
def check_for_toxicity(df, fragment_column, frag_mols):
    ...
    hepg2 = pd.read_excel(
        "../data/static_datasets/HEPG2_Primary_Toxicity_Data.xlsx", sheet_name="HepG2"
    )
    hepg2["hepg2_mean"] = hepg2["Mean_10uM"]
    primary = pd.read_excel(
        "../data/static_datasets/HEPG2_Primary_Toxicity_Data.xlsx", sheet_name="Primary"
    )
    ...
    df["average_hepg2_tox_score_of_matched_molecules"] = [
        np.mean(sublist) for sublist in list(df["hepg2_tox_matched_molecule_values"])
    ]
    df["average_primary_tox_score_of_matched_molecules"] = [
        np.mean(sublist) for sublist in list(df["primary_tox_matched_molecule_values"])
    ]
    return df
...
def check_full_cpd_similarity_to_closest_abx(
    ...
    tan_scores = [
        for_mol_list_get_lowest_tanimoto_to_closest_mol(
...
def process_molset(
    path, smi_col, hit_col="", just_actives=False, hit_thresh=0, greater_than=False
):
    if path == "":
...
def find_cpds_with_and_without_frags_for_whole_list(
    ...
    df["ttest_scos_with_and_without_frag"] = [
        sp.ttest_ind(x, y) for x, y in zip(list_of_scos_w_frag, list_of_scos_wo_frag)
    ]
    return df


def threshold_on_stat_sign_analogues_with_and_without_frags(
    df, absolute_diff_thresh=0, pval_diff_thresh=0
):
    ...
    if pval_diff_thresh > 0:
        ...
        print(
            ...
            + " (or n/a) stat significance between analogues w/ and w/o frag: ",
            len(df),
        )
    if absolute_diff_thresh > 0:
        df = threshold_on_absval_column(
            df, "average_difference_with_and_without_frag", absolute_diff_thresh
        )
        print(
            "number of fragments with >"
            + str(absolute_diff_thresh)
            + " (or n/a) absolute value difference between analogues w/ and w/o frag: ",
            len(df),
        )
    return df
    ...
    for i, row in df.iterrows():
        ...
        if len(full_mol_index_list) > 0:
            ...
            if len(abx_mols) > 0:
                curr_tan_abxs = [
                    float(x)
                    for x in list(row["tanimoto_scores_of_full_mols_to_nearest_abx"])
                ]
            else:
            ...
            if len(ts_mols) > 0:
                curr_tan_tss = [
                    float(x)
                    for x in list(
                        row["tanimoto_scores_of_full_mols_to_nearest_train_set"]
                    )
                ]
            else:
            ...
            curr_match_mols = [
                m for i, m in enumerate(cpd_mols) if i in curr_match_mol_index_list
            ]
            legends = [
                n
                + ", "
                ...
                for n, sc, ta, tt in zip(
                    curr_match_names, curr_match_scores, curr_tan_abxs, curr_tan_tss
                )
            ]
            if display_inline_candidates:
...
def filter_for_existing_mols(
    ...
    if test_name_needs_split:
        test_names = [
            x.split("-")[0] + "-" + x.split("-")[1] for x in list(testdf[test_name_col])
        ]
    else:
...
def deduplicate_on_similar_pairs(
    selected_mols_df, thresh=0.9, compound_smi_col="SMILES", compound_hit_col="hit"
):
    test1_cpds = [
        Chem.RDKFingerprint(Chem.MolFromSmiles(smi))
        for smi in list(selected_mols_df[compound_smi_col])
    ]
    dup_pairs = {}
    cpds_to_remove = (
        []
    )  # let's remove those with the lower predicted activity score (so close anyways, but needed some way to decide)
    for i, test1 in enumerate(test1_cpds):
        for j, test2 in enumerate(test1_cpds):
            ...
            if add_on:
                ...
                score_i = list(
                    selected_mols_df[
                        [x is i for x in list(range(len(selected_mols_df)))]
                    ][compound_hit_col]
                )[0]
                score_j = list(
                    selected_mols_df[
                        [x is j for x in list(range(len(selected_mols_df)))]
                    ][compound_hit_col]
                )[0]
                if score_i < score_j:
    ...
    dedup_selected_mols_df = selected_mols_df[
        [x not in cpds_to_remove for x in list(range(len(selected_mols_df)))]
    ]
...
def run_pipeline(
    ...
    current_all_matching_mols = [
        Chem.MolFromSmiles(smi) for smi in list(all_matching_mols_df[compound_smi_col])
    ]
    if len(abx_mols) > 0:
        all_matching_mols_df[
            "tan to nearest abx"
    ...
    mols = add_legends_to_compounds(
        all_matching_mols_df, smiles_column=compound_smi_col, name_column=cpd_name_col
    )
    df = extract_legends_and_plot(
        all_matching_mols_df,
        mols,
...
def process_fragments_after_receiving_available_compounds(
    ...
    if prim_frag_tox_cutoff > 0:
        ...
        print(
            "length of all frags with Primary tox < "
            + str(prim_frag_tox_cutoff)
            + ": ",
            len(df),
        )
    ...
    all_matching_mols_df[
        "tanimoto_scores_of_full_mols_to_nearest_abx"
    ] = for_mol_list_get_lowest_tanimoto_to_closest_mol(
        current_all_matching_mols, abx_mols
    )
    all_matching_mols_df[
        "tanimoto_scores_of_full_mols_to_nearest_train_set"
    ] = for_mol_list_get_lowest_tanimoto_to_closest_mol(
        current_all_matching_mols, ts_mols
    )
...
def check_full_cpd_similarity_to_closest_selected_mols(
    df, selected_mols_df, compound_smi_col
):
    if len(df) == 0:
        return []
...
def perform_filtering_algo(
    all_matching_mols_df,
    compound_smi_col,
    compound_hit_col,
    hepg2_cpd_col,
    prim_cpd_col,
):  # assumes a lot of information about hardcoded column names
    # go thru each fragment and pick best compound from that
    all_matching_mols_df["matching_frags_strings"] = [
        str(x) for x in list(all_matching_mols_df["matching_frags"])
    ]
    ...
    for frag_i, smalldf in all_matching_mols_df.groupby("matching_frags_strings"):
        ...
        smalldf[
            "tanimoto_scores_to_already_selected_mols"
        ] = check_full_cpd_similarity_to_closest_selected_mols(
            smalldf, selected_mols_df, compound_smi_col
        )
        smalldf = smalldf.sort_values(
            "tanimoto_scores_to_already_selected_mols", ascending=True
        )
        ...
        ):  # we haven't selected a fragment yet, so drop standards
            for i, row in smalldf.iterrows():
