...
sys.path.append(os.path.join(RDConfig.RDContribDir, 'SA_Score')) # now you can import sascore!
import sascorer
import cairosvg
...
def calculateScoreThruChemprop(patterns, results_folder_name, results_file_name, model_path, hit_column):
    ...
    if 'gonorrhea' in model_path:
        ...
        ft_command = 'python scripts/save_features.py --data_path ' + genML_folder + clean_name + ' --features_generator rdkit_2d_normalized --save_path ' + genML_folder + ft_name + '; '
        run_command = 'python predict.py --test_path ' + genML_folder + clean_name + ' --checkpoint_dir ' + model_path + ' --preds_path ' + genML_folder + clean_name + ' --features_path ' + genML_folder + ft_name + ' --no_features_scaling --smiles_columns SMILES'
        full_command = activate_command + ft_command + run_command
    else: # no features during training
        run_command = 'python predict.py --test_path ' + genML_folder + clean_name + ' --checkpoint_dir ' + model_path + ' --preds_path ' + genML_folder + clean_name + ' --features_generator rdkit_2d_normalized --no_features_scaling --smiles_columns SMILES'
...
def calculateScoreThruToxModel(patterns, results_folder_name, results_file_name, tox_model):
    ...
    run_command = 'python predict.py --test_path ' + genML_folder + clean_name + ' --checkpoint_dir ../' + model_folder + ' --preds_path ' + genML_folder + clean_name + ' --features_generator rdkit_2d_normalized --no_features_scaling --smiles_columns SMILES'
    full_command = activate_command + run_command
    test = subprocess.run(full_command, cwd="../models/chemprop-master/", shell=True, capture_output=True)
    print(test)
    preds = pd.read_csv(clean_name)
...
def select_top_based_on_criteria(smis, scores, regular_score = False, num_top_to_get = 5, num_random_to_get = 5):
    ...
    if not regular_score:
        ...
        # add tox score - hepg2
        _, hepg2_toxs = calculateScoreThruToxModel(list(sorteddf['SMILES']), '../out/crem/v6_adjusted_directed_score_02102023/', '_temp_predictions.csv', 'hepg2')
        sorteddf['hepg2_tox']= hepg2_toxs
        _, prim_toxs = calculateScoreThruToxModel(list(sorteddf['SMILES']), '../out/crem/v6_adjusted_directed_score_02102023/', '_temp_predictions.csv', 'primary')
    ...
    if len(sorteddf) < num_top_to_get+num_random_to_get:
        ...
        return([Chem.MolFromSmiles(smi) for smi in good_smis], good_scos, sorteddf, selecteddf)
...
def generate_molecules(mols, clean_dir, orig_frag_to_protect, round_num, grow_or_mut = 'grow', params = None, catalog = None, model_path = '', hit_column = 'hit'):
    print('-----------------------------')
    ...
    if grow_or_mut == 'grow':
        if params == None:
            ...
            min_atoms = params[1]
            radius = params[2]
            new_mols = [list(grow_mol(mol, db_fname, max_atoms=max_atoms, min_atoms=min_atoms, radius=radius, return_mol=True, ncores=16)) for mol in mols]
        ...
        if params == None:
            ...
            max_inc = params[3]
            radius = params[4]
            new_mols = [list(mutate_mol(Chem.AddHs(mol), db_name=db_fname, return_mol=True, min_size=min_size, max_size=max_size, min_inc=min_inc, max_inc=max_inc, radius=radius, ncores=16)) for mol in mols]
    ...
    img = Draw.MolsToGridImage(new_mols, molsPerRow=10,maxMols=50, useSVG=True) # throws weird error if empty list is provided
    clean_name = clean_dir + 'all_mols_round_' + str(round_num)
    cairosvg.svg2png(img.data, write_to=clean_name+'.png')
    
    # get new scores
    smis, scores = get_molecule_scores(new_mols, clean_dir + 'all_mols_round_' + str(round_num), model_path, hit_column) # doesn't include the previous round
    best_score = max(scores)
    print('molecules generated containing fragment + passing PAINS, Brenk filters:', len(new_mols))
    print('best score:', np.round(best_score, 3))
    
    return(smis, scores)

def run_crem(out_dir, orig_frag_smi, orig_mol_smi, max_atom_range, min_atom_range, radius_range, min_inc_range=[2], max_inc_range=[-2], num_iters=5, method='grow', regular_score=True, num_top_to_get=5, num_random_to_get=5, cpd_filter='both', model_path = '', hit_column = 'hit'):
    ...
    if method == 'grow':
        # grow params
        for ma in max_atom_range: # max_atoms – maximum number of atoms in the fragment which will replace H
            for mi in min_atom_range: # min_atoms – minimum number of atoms in the fragment which will replace H
                for ra in radius_range: # radius – radius of context which will be considered for replacement
                    param_list.append([ma,mi,ra])
    else:
        # mutate params
        for mi_s in min_atom_range: # min_size – minimum number of heavy atoms in a fragment to replace. If 0 - hydrogens will be replaced (if they are explicit)
            for ma_s in max_atom_range: # max_size – maximum number of heavy atoms in a fragment to replace
                for mi in min_inc_range: # min_inc – minimum change of a number of heavy atoms in replacing fragments to a number of heavy atoms in replaced one. Negative value means that the replacing fragments would be smaller than the replaced one on a specified number of heavy atoms
                    for ma in max_inc_range: # max_inc – maximum change of a number of heavy atoms in replacing fragments to a number of heavy atoms in replaced one
                        for ra in radius_range: # radius – radius of context which will be considered for replacement
    ...
    for param_set in tqdm.tqdm(param_list):
        ...
        if method == 'grow':
            ...
            name = str(param_set[0]) + '_minsize_' + str(param_set[1]) + '_maxsize_' + str(param_set[2]) + '_mininc_' + str(param_set[3]) + '_maxinc_' + str(param_set[4]) + '_radius'
        ...
        _, orig_mol_scos = calculateScoreThruChemprop([orig_mol_smi], out_dir, 'original_starting_mols_round_0.csv', model_path, hit_column)
        ...
        for i in range(num_iters):
            ...
            smis, scores = generate_molecules(selected_top_mols_to_mutate, clean_dir, orig_frag_smi, i, grow_or_mut = method, params = param_set, catalog=catalog, model_path=model_path, hit_column=hit_column)

            # select top compounds to continue with
            if np.max(param_set) >= 8: # any parameters too big, must reduce the size
                num_top_to_get = 2
                num_random_to_get = 1
            selected_top_mols_to_mutate, selected_top_mol_scos, alldf, selecteddf = select_top_based_on_criteria(smis, scores, regular_score = regular_score, num_top_to_get=num_top_to_get, num_random_to_get=num_random_to_get)

            # save the last mols generated
            alldf.to_csv(clean_dir + 'all_mols_from_round_' + str(i) + '.csv', index = False)
            selecteddf.to_csv(clean_dir + 'original_starting_mols_round_' + str(i+1) + '.csv', index = False)