...
def make_tsne_figure(tsne_df, fig_path, colors=None):
    ...
    ax = sns.scatterplot(
        data=tsne_df, x="X", y="Y", hue="label", palette=palette, alpha=0.7, s=4
    )
...
def evaluate_model(
    validation, cutoff_for_positive, actual_col="class", predicted_col="ACTIVITY"
):
...
def draw_mols_with_highlight(
    ...
    d2d.DrawMolecules(
        ...
        highlightAtomColors=hit_atom_cols,
        highlightBondColors=hit_bond_cols,
    )
    d2d.FinishDrawing()
...
def interpretation_increase_resolution(
    ...
    corrections_dict = {
        "C[C@]1(Cn2ccnn2)C(N2C(CC2=O)S1(=O)=O)C(O)=O.O=C1CC2N1C([CH3:1])[C:1][S:1]2": (
            1,
            8,
            9,
            10,
